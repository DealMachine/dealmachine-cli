...
for (const file of artifact.files) {
  assert.doesNotMatch(
    file.path,
    /(^|\/)(private|v2|opportunities|opportunity-fields|opportunityActivities|suppression|contactExclusions)(\/|\.)/,
  ...
  assert.doesNotMatch(
    content,
    /registerV2Commands|registerBetaCommands|registerOpportunitiesCommands|include_companies|company_limit|V2 Query Workflows|dm query schema|\/query\/schema|\/opportunities(?:\/|['"`])|\/suppression-list|exclude[-_](?:exported|prospect|listed|suppressed)[-_]contacts|include[-_]properties[-_]without[-_]contacts/,
...
for (const command of ['query', 'companies', 'opportunities', 'opp', 'suppression']) {
  const result = spawnSync(process.execPath, [resolve(cli, 'dist/index.js'), command], {
    cwd: cli,
    encoding: 'utf8',
    env: { ...process.env, DM_ENABLE_OPPORTUNITIES: 'true' },
  });
